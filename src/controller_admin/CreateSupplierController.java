...
public class CreateSupplierController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            ...
            Logger.getLogger(CreateSupplierController.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void createSupplierClicked(MouseEvent event) throws SQLException {
        if (isAddressRight() && isCompanyNameRight() && isPhoneRight()) {
            ...
            if (new SupplierModifier().createSupplier(supplier)) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Notification");
                alert.setHeaderText("Success");
                alert.setContentText("Create supplier is successfully.");
                alert.showAndWait();
                getListSupplier();
            }
        } else {
